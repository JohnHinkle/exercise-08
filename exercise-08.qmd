...
The regression coefficients are different between the three groups.
...
**What is it that we are permuting?** The slope coefficient
...
```{r}
perm_p_value <- sum(perm$stat > abs(obs_slope) | perm$stat < -1 * abs(obs_slope)) / n_perm
